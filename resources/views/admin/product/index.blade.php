        ...
        <div class="col-lg-12 mb-12">
          <div class="card">
            <div class="card-header row">
              ...
              <a href="{{ route('admin.product.create') }}" class="btn btn-primary col-2 float-right">Add New Product</a>
            </div>
              ...
              <table id="product_table" class="table table-striped table-hover card-text">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td><img src="{{ $value->product_image }}" alt="" width="100" height="50"></td>
                    <td>{{ $value->name }}</td>
                    <td>{{ $value->price }}</td>
                    ...
                    <td>
                      <a href="{{ route('admin.product.edit', $value->id) }}" class="text-primary fs-6 mr-1" data-toggle="tooltip" title="Edit">
                          <i class="fas fa-edit"></i>
                      </a>
                      <a onclick="DeleteProduct({{ $value->id }})" class="cursor-pointer"><i class="fa fa-trash" aria-hidden="true"></i></a>
                    </td>
                  </tr>
                  @endforeach
                </tbody>
  ...
  function productStatus(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          type: "POST",
          url: "{{ route('admin.product.statusUpdate', ['id' => ':id']) }}".replace(':id', id),
          data: {
            _token: "{{ csrf_token() }}"
          },
          success: function(response) {
