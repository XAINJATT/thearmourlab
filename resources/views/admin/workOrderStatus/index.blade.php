                    ...
                    <div class="col-lg-12 mb-12">
                        <div class="card">
                            <div class="card-header row">
                                ...
                                <a style="min-width:250px" href="{{ route('admin.WorkOrderStatus.create') }}"
                                    class="btn btn-primary col-3 float-right">Add New Work Order Status</a>
                            </div>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    @elseif($value->status == 'Paint Protection Film')
                                                        <a onclick="workOrderStatus({{ $value->id }})">
                                                            <span class="badge bg-secondary cursor-pointer">Paint Protection
                                                                Film</span>
                                                        </a>
                                                    @elseif($value->status == 'Ceramic Coating')
                                                        <a onclick="workOrderStatus({{ $value->id }})">
                                                            <span class="badge bg-info cursor-pointer">Ceramic
                                                                Coating</span>
                                                        </a>
                                                ...
                                                <td>
                                                    <a href="{{ route('admin.WorkOrderStatus.edit', $value->id) }}"
                                                        class="text-primary fs-6 mr-1" data-toggle="tooltip" title="Edit">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <a onclick="DeleteWorkOrderStatus({{ $value->id }})"
        ...
        function DeleteWorkOrderStatus(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "POST",
                        url: "{{ url('admin/WorkOrderStatus/delete') }}" + '/' +
                        id, // Pass the WorkOrderStatus parameter
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
        ...
        function workOrderStatus(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('admin.WorkOrderStatus.statusUpdate', ['id' => ':id']) }}".replace(
                            ':id', id),
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
