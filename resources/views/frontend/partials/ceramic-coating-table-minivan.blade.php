<table class="table table-hover packages">
    <tr class="package-list">
        <th data-val="val1">@lang("messages.Packages")</th>
        <th data-val="val2">@lang("messages.POLY")</th>
        <th data-val="val3">@lang("messages.QUARTZ")</th>
        <th data-val="val4">@lang("messages.QUARTZ") +</th>
        <th data-val="val5">@lang("messages.KENZO")</th>
        <th data-val="val6">@lang("messages.INTERIOR")</th>
        <th data-val="val7">@lang("messages.MAINTENANCE WASH")</th>
    </tr>
    <tr class="package-list">
        <td>@lang("messages.Hardness of coating")</td>
        <td>8H</td>
        <td>9H</td>
        <td>9H @lang("messages.GRAPHENE INFUSED")</td>
        <td>10H @lang("messages.GRAPHENE INFUSED")</td>
        <td>✖</td>
        <td>✖</td>
    </tr>

    <tr class="package-list">
        <td>@lang("messages.PRICE")</td>
        <td class="price">$449.99</td>
        <td class="price">$749.99</td>
        <td class="price">$1049.99</td>
        <td class="price">$1249.99</td>
        <td class="price">$449.99</td>
        <td class="price">$249.99</td>
    </tr>
    <tr class="package-list">
        <td>@lang("messages.ESTIMATED TIME")</td>
        <td>24 HRS</td>
        <td>24 HRS</td>
        <td>48 HRS</td>
        <td>48 HRS</td>
        <td>3-5 HRS</td>
        <td>8 HRS</td>
    </tr>

    <tr class="package-list">
        <td>@lang("messages.Warranty")</td>
        <td> 1 @lang("messages.Year")<br /></td>
        <td> 2 @lang("messages.Years")<br /></td>
        <td> 4 @lang("messages.Years")<br /></td>
        <td> 5 @lang("messages.Years")<br /></td>
        <td> ✖<br /></td>
        <td> ✖<br /></td>
    </tr>

    <tr class="package-list">
        <td>@lang("messages.TWO BUCKET WASH")</td>
        <td> ✔</td>
        <td> ✔</td>
        <td> ✔</td>
        <td>✔</td>
        <td>✖</td>
        <td> ✔</td>
    </tr>
    <tr class="package-list">
        <td>@lang("messages.CLAY BAR TREATMENT")</td>
        <td> ✔</td>
        <td> ✔</td>
        <td> ✔</td>
        <td>✔</td>
        <td>✖</td>
        <td> ✔</td>
    </tr>
    <tr class="package-list">
        <td>@lang("messages.IRON AND DECON REMOVAL")</td>
        <td> ✔</td>
        <td> ✔</td>
        <td> ✔</td>
        <td>✔</td>
        <td> ✖</td>
        <td> ✔</td>
    </tr>
    <tr class="package-list">
        <td>@lang("messages.ONE STAGE SPEED POLISH")</td>
        <td>✖</td>
        <td> ✔</td>
        <td> ✔</td>
        <td>✔</td>
        <td>✖</td>
        <td> ✖</td>
    </tr>
    <tr class="package-list">
        <td>@lang("messages.Paint Correction (PER STAGE)")</td>
        <td>
            $599.99
        </td>
        <td>
            $599.99
        </td>
        <td>
            $599.99
        </td>
        <td>$599.99</td>
        <td>✖</td>
        <td>✖</td>
    </tr>
    <tr class="package-list">
        <td>@lang("messages.Window coating")</td>
        <td>✖</td>
        <td> ✔</td>
        <td> ✔</td>
        <td> ✔</td>
        <td> ✔</td>
        <td> ✔</td>
    </tr>
    <tr class="package-list">
        <td>@lang("messages.Wheel coating")</td>
        <td>✖</td>
        <td> ✔</td>
        <td> ✔</td>
        <td> ✔</td>
        <td>✖</td>
        <td>✖</td>
    </tr>
    <tr class="package-list">
        <td>@lang("messages.Full body coating")</td>
        <td> ✔</td>
        <td> ✔</td>
        <td> ✔</td>
        <td> ✔</td>
        <td>✖</td>
        <td>✖</td>
    </tr>
    <tr class="package-list">
        <td>@lang("messages.Premier coating")</td>
        <td>✖</td>
        <td>✖</td>
        <td>✖</td>
        <td>✖</td>
        <td>✖</td>
        <td>✔</td>
    </tr>
    <tr class="package-list">
        <td>@lang("messages.Trim / Exhaust / Misc coating")</td>
        <td>✖</td>
        <td> ✔</td>
        <td> ✔</td>
        <td> ✔</td>
        <td>✖</td>
        <td>✖</td>
    </tr>
    <tr class="package-list">
        <td>@lang("messages.Wheels off package")</td>
        <td>✖</td>
        <!-- <td>
            $199.99
        </td> -->
        <td>
            $249.99
        </td>
        <td>
            $249.99
        </td>
        <td>
            $249.99
        </td>
        <td>✖</td>
        <td>✖</td>
    </tr>
</table>
